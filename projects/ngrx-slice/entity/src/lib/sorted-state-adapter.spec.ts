...
describe(createSortedStateAdapter.name, () => {
  ...
  it('should remove existing and add new ones with setAll', () => {
    ...
    expect(setAllResult).toEqual({
      ids: [publishSliceTodo.id, writeTestTodo.id],
      entities: {
        [writeTestTodo.id]: writeTestTodo,
  ...
  it('should not change ids if an update does not impact sorting', () => {
    const setAllResult = adapter.setAll(state, [
      writeSliceTodo,
      writeTestTodo,
      publishSliceTodo,
    ...
    const updateOneResult = adapter.updateOne(setAllResult, {
      id: writeTestTodo.id,
      changes: { title: 'Wrote Test' },
    });

    expect(updateOneResult.ids).toBe(setAllResult.ids);
  });
  ...
  it('should let you update many entities in the state', () => {
    ...
    expect(updateManyResult).toEqual({
      ids: [publishSliceTodo.id, writeSliceTodo.id, writeTestTodo.id],
      entities: {
        [publishSliceTodo.id]: publishSliceTodo,
        [writeSliceTodo.id]: { ...writeSliceTodo, title: 'Wrote Slice' },
  ...
  it('should let you add and update entities with upsertMany', () => {
    ...
    expect(upsertManyResult).toEqual({
      ids: [writeTestTodo.id, writeSliceTodo.id],
      entities: {
        [writeSliceTodo.id]: { ...writeSliceTodo, title: 'Wrote Slice' },
        [writeTestTodo.id]: writeTestTodo,
      },
